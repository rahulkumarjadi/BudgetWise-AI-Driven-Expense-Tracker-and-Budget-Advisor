...
//
//   const [loading, setLoading] = useState(false);   // ← false: no spinner on refresh
//   const [hasFetched, setHasFetched] = useState(false); // ← controls empty state
//   const [errorMessage, setErrorMessage] = useState("");
//
//  const currentUserId = user?.id || user?.subject || user?.userId || user?._id;
//
//   const theme = {
//     bg: isDark ? "#0f172a" : "#f8fafc",
//     card: isDark ? "rgba(30, 41, 59, 0.7)" : "rgba(255, 255, 255, 0.9)",
...
//       setOverallBudget(overallRes.data || { totalBudget: 0, totalSpent: 0 });
//       setSavingsGoals(Array.isArray(savingsRes.data) ? savingsRes.data : []);
//       setHasFetched(true); // ← show dashboard after first successful fetch
//     } catch (err) {
//       console.error("Analytics Fetch Error:", err);
//       setErrorMessage("Data synchronization failed.");
//     } finally {
//       setLoading(false);
//     }
//   }, [month, year, category, currentUserId]);
//
//   const fetchAIInsights = async () => {
//     if (!currentUserId) return;
//
...
//   };
//
//   // ← NO useEffect: data only loads when user clicks "Update View"
//
//   const incomeExpenseChart = [
//     { name: "Total Revenue", value: Number(incomeExpense.income) || 0, fill: "#10b981" },
//     { name: "Total Outflow", value: Number(incomeExpense.expense) || 0, fill: "#f43f5e" }
...
//           to { opacity: 1; transform: translateY(0); }
//         }
//         .ai-panel { animation: slideIn 0.3s ease-out; }
//         .insight-card { animation: fadeIn 0.5s ease-out; }
//       `}</style>
//
//       <Sidebar />
//       <main style={styles.page}>
...
//               }}
//             >
//               {aiLoading ? <>🤖 Analyzing...</> : showAIInsights ? <>✨ Hide AI Insights</> : <>🧠 Gemini AI Insights</>}
//             </button>
//           </div>
//         </div>
//
...
//           <div style={styles.loaderWrap}><div style={styles.pulse}></div></div>
//         ) : !hasFetched ? (
//           // ── Empty state shown on refresh until user clicks "Update View" ──
...
//             <h2 style={{fontSize:'1.5rem', fontWeight:'800', margin:0, color: theme.text}}>No Data Loaded</h2>
//             <p style={{color: theme.subText, fontSize:'0.95rem', margin:0}}>
//               Select your filters and click <strong style={{color: theme.accent}}>Update View</strong> to load your analytics.
//             </p>
//           </div>
//         ) : (
//           <div style={{display: 'flex', gap: '25px'}}>
...
//                       ? <p style={{color: theme.subText, fontSize: '0.8rem'}}>Zero active targets.</p>
//                       : savingsGoals.map((goal) => (
//                         <div key={goal.id} style={styles.goalRow}>
//                           <div style={styles.goalInfo}>
//                             <span style={{fontSize: '0.85rem'}}>{goal.name}</span>
...
//
//                 <div style={styles.tabContainer}>
//                   <button onClick={() => setActiveTab('gemini')} style={{...styles.tab, background: activeTab === 'gemini' ? 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' : 'transparent', color: activeTab === 'gemini' ? '#fff' : theme.text}}>✨ Gemini AI</button>
//                   <button onClick={() => setActiveTab('smart')} style={{...styles.tab, background: activeTab === 'smart' ? theme.accent : 'transparent', color: activeTab === 'smart' ? '#fff' : theme.text}}>📊 Analytics</button>
//                 </div>
//
//                 <div style={styles.aiContent}>
//                   {activeTab === 'gemini' && geminiInsights && (
...
//                             <svg width="100" height="100" style={{transform: 'rotate(-90deg)'}}>
//                               <circle cx="50" cy="50" r="40" stroke={isDark ? '#1e293b' : '#e2e8f0'} strokeWidth="8" fill="none" />
//                               <circle cx="50" cy="50" r="40" stroke={getHealthScoreColor(geminiInsights.financialHealthScore)} strokeWidth="8" fill="none" strokeDasharray={`${(geminiInsights.financialHealthScore / 100) * 251.2} 251.2`} strokeLinecap="round" />
//                             </svg>
//                             <div style={{position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center'}}>
//                               <div style={{fontSize: '1.5rem', fontWeight: '800', color: getHealthScoreColor(geminiInsights.financialHealthScore)}}>{geminiInsights.financialHealthScore}</div>
//                             </div>
//                           </div>
//                           <div>
//                             <div style={{fontSize: '1.2rem', fontWeight: '700', color: getHealthScoreColor(geminiInsights.financialHealthScore)}}>{getHealthScoreLabel(geminiInsights.financialHealthScore)}</div>
//                             <p style={{fontSize: '0.75rem', color: theme.subText, margin: '5px 0 0 0', lineHeight: '1.5'}}>{geminiInsights.healthAssessment}</p>
//                           </div>
//                         </div>
//                       </div>
//
//                       {geminiInsights.recommendations?.length > 0 && (
//                         <div className="insight-card" style={{...styles.insightCard, background: theme.bg}}>
//                           <h4 style={{fontSize: '0.9rem', color: theme.subText, margin: '0 0 15px 0'}}>💡 AI Recommendations</h4>
//                           {geminiInsights.recommendations.map((rec, idx) => (
//                             <div key={idx} style={{marginBottom: '12px', padding: '12px', background: 'rgba(99, 102, 241, 0.05)', borderRadius: '8px', borderLeft: `3px solid ${theme.accent}`}}>
//                               <div style={{display: 'flex', gap: '10px', alignItems: 'flex-start'}}>
//                                 <span style={{fontSize: '1.2rem', marginTop: '2px'}}>{idx === 0 ? '🎯' : idx === 1 ? '💰' : idx === 2 ? '📈' : '✅'}</span>
//                                 <p style={{fontSize: '0.85rem', color: theme.text, margin: 0, lineHeight: '1.6'}}>{rec}</p>
//                               </div>
//                             </div>
//                           ))}
...
//                         <div className="insight-card" style={{...styles.insightCard, background: theme.bg}}>
//                           <h4 style={{fontSize: '0.9rem', color: theme.subText, margin: '0 0 15px 0'}}>🔍 Spending Habits</h4>
//                           <p style={{fontSize: '0.85rem', color: theme.text, lineHeight: '1.7', margin: 0}}>{geminiInsights.spendingHabitsAnalysis}</p>
//                         </div>
//                       )}
//
//                       {geminiInsights.longTermStrategy && (
//                         <div className="insight-card" style={{...styles.insightCard, background: theme.bg}}>
//                           <h4 style={{fontSize: '0.9rem', color: theme.subText, margin: '0 0 15px 0'}}>🚀 Long-term Strategy</h4>
//                           <p style={{fontSize: '0.85rem', color: theme.text, lineHeight: '1.7', margin: 0}}>{geminiInsights.longTermStrategy}</p>
//                         </div>
//                       )}
//
//                       {geminiInsights.personalizedTips?.length > 0 && (
//                         <div className="insight-card" style={{...styles.insightCard, background: theme.bg}}>
//                           <h4 style={{fontSize: '0.9rem', color: theme.subText, margin: '0 0 15px 0'}}>💎 Pro Tips</h4>
//                           {geminiInsights.personalizedTips.map((tip, idx) => (
//                             <div key={idx} style={{padding: '10px 12px', background: 'rgba(16, 185, 129, 0.05)', borderRadius: '8px', marginBottom: '8px', fontSize: '0.8rem', color: theme.text, lineHeight: '1.5'}}>{tip}</div>
//                           ))}
//                         </div>
//                       )}
//                     </>
...
//                             </div>
//                           </div>
//                           <div style={{...styles.pill, background: 'rgba(99, 102, 241, 0.1)', color: theme.accent, justifyContent: 'center'}}>Trend: {aiInsights.expenseForecast.trend}</div>
//                         </div>
//                       )}
//
//                       {aiInsights.savingsRecommendations?.length > 0 && (
//                         <div className="insight-card" style={{...styles.insightCard, background: theme.bg}}>
//                           <h4 style={{fontSize: '0.9rem', color: theme.subText, margin: '0 0 15px 0'}}>💡 Savings Opportunities</h4>
//                           {aiInsights.savingsRecommendations.map((rec, idx) => (
//                             <div key={idx} style={{marginBottom: '15px', padding: '12px', background: 'rgba(16, 185, 129, 0.05)', borderRadius: '8px', borderLeft: `3px solid ${getPriorityColor(rec.priority)}`}}>
//                               <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px'}}>
//                                 <span style={{fontWeight: '700', fontSize: '0.85rem'}}>{rec.category}</span>
//                                 <span style={{...styles.priorityBadge, background: getPriorityColor(rec.priority), color: '#fff'}}>{rec.priority}</span>
//                               </div>
...
//   pulse: { width: '40px', height: '40px', borderRadius: '50%', background: '#6366f1', animation: 'pulse 1.5s infinite ease-in-out' },
//   errorToast: { padding: '15px', background: '#fef2f2', color: '#dc2626', borderRadius: '12px', border: '1px solid #fee2e2', marginBottom: '20px' },
//   aiPanel: { flex: '0 0 40%', border: '1px solid', borderRadius: '24px', padding: '28px', maxHeight: 'calc(100vh - 140px)', overflowY: 'auto', boxShadow: "0 10px 40px -5px rgba(0,0,0,0.1)" },
//   aiHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', paddingBottom: '15px', borderBottom: '1px solid rgba(99, 102, 241, 0.2)' },
//   closeBtn: { background: 'rgba(239, 68, 68, 0.1)', border: 'none', borderRadius: '50%', width: '32px', height: '32px', cursor: 'pointer', fontSize: '1.2rem', color: '#ef4444', display: 'flex', alignItems: 'center', justifyContent: 'center' },
//   tabContainer: { display: 'flex', gap: '10px', marginBottom: '20px', padding: '5px', background: 'rgba(0,0,0,0.02)', borderRadius: '12px' },
//   tab: { flex: 1, padding: '10px 16px', border: 'none', borderRadius: '8px', fontWeight: '700', fontSize: '0.85rem', cursor: 'pointer', transition: 'all 0.2s ease' },
//   aiContent: { display: 'flex', flexDirection: 'column', gap: '20px' },
//   insightCard: { padding: '20px', borderRadius: '16px', border: '1px solid rgba(99, 102, 241, 0.1)', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' },
//   priorityBadge: { padding: '3px 10px', borderRadius: '12px', fontSize: '0.7rem', fontWeight: '800' }
// };
